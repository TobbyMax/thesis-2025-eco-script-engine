...
#include "IEcoMap1.h"
#include "IEcoBase1.h"
#include "IEcoAST1Declaration.h"
#include "IEcoAST1Statement.h"
#include "IEcoAST1Type.h"
#include "IEcoAST1.h"
#include "IEcoAST1Scope.h"
#include "IEcoAST1GlobalScope.h"
#include "IEcoAST1StructOrUnionDeclaration.h"
#include "IEcoAST1StructOrUnionType.h"
#include "IEcoAST1EnumDeclaration.h"
#include "IEcoAST1EnumType.h"
...
bool_t OutputDiagnosticInfoOfLRk(/* in */ uint16_t langId, /* in */ IEcoLog1* pILog, /* in */ IEcoMemoryAllocator1* pIMem, /* in */ IEcoString1* pIStr, /* in */ IEcoParser1LRk* pILRk) {
    ...
    for (i = 0; i < pIItemSets->pVTbl->Count(pIItemSets); ++i) {
        ...
        for (pISetIter = pIItems->pVTbl->Get_IterBegin(pIItems); pISetIter->pVTbl->Get_Value(pISetIter) != 0; pISetIter->pVTbl->GoToNext(pISetIter)) {
            pIItem = (IEcoParser1LRkItem*)pISetIter->pVTbl->Get_Value(pISetIter);
    ...
    for (i = 0; i < pIColumns->pVTbl->Count(pIColumns); ++i) {
        ...
        for (j = 0; j < pIRows->pVTbl->Count(pIRows); ++j) {
            ...
            pIAction = (IEcoParser1Action*)pIRow->pVTbl->get_ItemByIndex(pIRow, i);
    ...
    for (i = 0; i < pIRows->pVTbl->Count(pIRows); ++i) {
        ...
        for (j = 0; j < pIColumns->pVTbl->Count(pIColumns); ++j) {
            ...
            pIAction = (IEcoParser1Action*)pIRow->pVTbl->get_ItemByIndex(pIRow, j);
 ...
 *
 * <сводка>
 *   Функция OutputDiagnosticInfoOfASTNode
 * </сводка>
 *
 * <описание>
 *   Функция OutputDiagnosticInfoOfASTNode - Вывод диагностической информации узла дерева
 * </описание>
 *
 */
bool_t OutputDiagnosticInfoOfASTNode(/* in */ IEcoLog1* pILog, /* in */ IEcoMemoryAllocator1* pIMem, /* in */ IEcoString1* pIStr, /* in */ IEcoTree1Node* pINode, char_t* offset) {
    ...
    IEcoAST1SyntaxObject* pIObject = 0;
    ...
    char* name = pINode->pVTbl->get_Name(pINode);
    printf("%s|_ %s\n", offset, pINode->pVTbl->get_Name(pINode));
    ...
    if (result == 0 && pIScope != 0) {
        ...
        printf("%s|     Struct or union declarations: %s\n", offset, description);
...
bool_t OutputDiagnosticInfoOfAST(/* in */ uint16_t langId, /* in */ IEcoLog1* pILog, /* in */ IEcoMemoryAllocator1* pIMem, /* in */ IEcoString1* pIStr, /* in */ IEcoAST1* pIAST) {
    ...
    for (i = 0; i < pIForest->pVTbl->Count(pIForest); ++i) {
        ...
        if (pINode->pVTbl->set_Data == 0) {
            printf("WARNING! Tree[%i] is has corrupt root Node\n", i);
            continue;
        }
